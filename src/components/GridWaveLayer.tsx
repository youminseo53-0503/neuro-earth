...
const SURF = EARTH_RADIUS * 1.013; // 구름 셸(1.012·opacity 0.32) '위' + 항공망(1.014) '아래' = 지표 기는 기반 인프라
// (1.005는 구름 안쪽이라 반투명 구름에 색이 탁해졌음 / 0.997은 지표 안쪽이라 지구에 가려졌음)
const NODE_SIZE = 0.009; // 점은 작게 — 선(흐름)이 주인공
const REST_SCALE = 0.22; // 점 크기는 거의 고정(투명도로 세기 표현, 크기로 X)
const GROW_SCALE = 0.05; // 흥분해도 크기는 '거의' 고정(미세 숨결만) — 세기는 알파로
...
export function GridWaveLayer() {
  const { net, sources, nodeMatrices, lineGeom, lineMat, vis, flowBuf } = useMemo(() => {
    const net = new PlasticityNetwork({ decayPrune: true }); // 안 쓰인 길은 시간이 지나며 진짜 사라짐
    const sources = makeSources(["starlink"]);
    const vis = new Float32Array(net.nodes.length); // 노드별 부드러운 시각 활성(0..1)
    const flowBuf = new Float32Array(net.synapses.length); // 시냅스별 느린 감쇠 흐름(혜성 꼬리·표시 전용)
    ...
    return { net, sources, nodeMatrices, lineGeom, lineMat, vis, flowBuf };
  }, []);
  ...
  useFrame(() => {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      // 작은 점 — idle엔 거의 검정(줄전구 방지), 흐름 지날 때만 보라로 떠오름(선이 주인공)
      color.setRGB(0.02 + v * 0.58, 0.01 + v * 0.30, 0.04 + v * 0.76);
      mesh.setColorAt(i, color);
      // 흥분 전엔 거의 안 보이는 점 → 흐름 지날 때만 떠오름(크기 거의 고정, 세기는 색/알파)
      dummy.position.set(n.x * SURF, n.y * SURF, n.z * SURF);
