...
/**
 * 그리드 파동 레이어 — 고정 격자 신경망('이미 깔린 통신 인프라') 위로
 * 인터넷 정보(Wikimedia 실시간 편집)가 파동처럼 계속 이동한다. 지금 이 순간
 * 전 세계가 만드는/고치는 위키 편집이 그 언어 지역 격자를 때리고, 파동이 망을 타고 번진다.
 * 항공망(emergent)과 독립적으로 굴러가고 보라색으로 구분. 사회현상: 네트워크로 퍼지는 정보.
 */
export function GridWaveLayer() {
  const { net, sources, nodeMatrices, lineGeom, lineMat } = useMemo(() => {
    const net = new PlasticityNetwork();
    const sources = makeSources(["netinfo"]);
